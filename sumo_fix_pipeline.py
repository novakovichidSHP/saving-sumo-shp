...
# --- Корректная обработка всех встроенных base64-изображений ---
def convert_all_embedded_images_to_png(data, temp_dir, original_data=None):
    ...
    error_images = 0
    img_counter = 0
    def recursive_convert(obj, orig_obj=None):
        nonlocal converted, validated_png, error_images, img_counter
                    ...
                    if m:
                        ...
                        try:
                            ...
                            if fmt != 'png':
                                ...
                                new_str = 'data:image/png;base64,' + png_b64
                                if not isinstance(new_str, str):
                                    raise ValueError('Результат base64 не строка!')
                                obj[k] = new_str
                                converted += 1
                            else:
                                # Проверяем, что строка действительно декодируется и открывается как PNG
                                try:
                                    test_img = Image.open(BytesIO(img_bytes))
                                    if test_img.format != 'PNG':
                                        # Принудительно перекодируем в PNG
                                        buf = BytesIO()
                                        test_img.convert('RGBA').save(buf, format='PNG')
                                        png_b64 = base64.b64encode(buf.getvalue()).decode('utf-8')
                                        obj[k] = 'data:image/png;base64,' + png_b64
                                        converted += 1
                                    else:
                                        validated_png += 1
                                except Exception as e:
                                    print(f'Ошибка проверки PNG: {e}')
                                    if orig_v:
                                        obj[k] = orig_v
                                    error_images += 1
                        except Exception as e:
                            print(f'Ошибка обработки изображения (format={fmt}): {e}')
                            if orig_v:
                                obj[k] = orig_v
                            error_images += 1
                    else:
                        recursive_convert(v, orig_v)
                else:
                    recursive_convert(v, orig_v)
        elif isinstance(obj, list):
            for idx, item in enumerate(obj):
                orig_item = orig_obj[idx] if (orig_obj and isinstance(orig_obj, list) and idx < len(orig_obj)) else None
                recursive_convert(item, orig_item)
    recursive_convert(data, original_data)
    print(f'Найдено форматов изображений: {found_formats}')
    print(f'Преобразовано в PNG: {converted}, валидировано PNG: {validated_png}, ошибок: {error_images}')
    return data
...
def process_file(sumo_path, out_path):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        data = convert_all_embedded_images_to_png(data, temp_dir, original_data)
